import {resultOverview,outcomeLabels,filterOverviewRows} from './result-overview.js';
...
export async function mountOperations(parent,request,isCurrent=()=>true){
 ...
 const results=button(live,'View results',async()=>{let id=runSnapshot?.id;if(!id){const page=await request('v2-operations/runs?limit=50&offset=0');id=page.rows?.find(r=>r.jobId===trackedId)?.id;}selectedRun=id??null;tab='Runs';await render();});results.disabled=true;
 // Automatic updates use only the bounded job read model. Result snapshots load on manual navigation/refresh.
 const trackJob=(job,receipt=true,knownRun=null,knownServices=null)=>{
  if(disposed||!isCurrent())return;
  if(trackedId===job.id&&tracking){if(knownRun){runSnapshot=knownRun;serviceSnapshot=knownServices;}if(lastJob)showLive(lastJob);return;}
  stopMonitor();trackedId=job.id;tracking=true;lastJob=null;runSnapshot=knownRun;serviceSnapshot=knownServices;lastAnnounced=null;live.hidden=false;
  for(const node of Object.values(liveFields))updateText(node,'');
  updateText(liveFields.receipt,(receipt?'Admission receipt for job ':'Monitoring job ')+job.id+'. Checking live status…');showLive(job);results.disabled=false;
  stopMonitor=monitorJob(job.id,request,(current,error)=>{
   if(error){updateText(liveFields.notice,error);return;}
   lastJob=current;updateText(liveFields.receipt,'Authoritative Operations status; counters are retained run snapshots.');updateText(liveFields.notice,'');showLive(current);
 ...
 async function explorer(p,unresolved=false,recordedRows=null){
  let selectedIds=null;const selection=el('p','All services');p.append(selection);
  ...
  button(form,'Apply',async()=>{query=q.value;filter=state.value;offset=0;await load();});const select=async(label,ids)=>{selectedIds=ids?new Set(ids):null;query='';filter='';q.value='';state.value='';offset=0;selection.textContent=label;await load();p.scrollIntoView?.({block:'start'});};if(recordedRows)button(form,'Show all services',()=>select('All services',null));await load();return select;
 }
 ...
 function renderOverview(p,run,rows,select,reportedTotal){
  const model=resultOverview(run,rows),metric=(parent,label,count,ids)=>button(parent,`${label}: ${count}`,()=>select(label,ids));
  p.append(el('h2','Result overview'));text(p,`${model.total} services reported${reportedTotal>rows.length?' shown — partial historical projection':''}`);
  const outcomes=el('div',undefined,'ops-overview-metrics');p.append(outcomes);
  for(const [key,label]of Object.entries(outcomeLabels))metric(outcomes,label,model.buckets[key].length,model.buckets[key]);
  text(p,'Exclusive service outcomes sum to Total. No approval or admission is inferred. Partial and pending evidence are not failures; pricing is independent. Other / not reported retains states without a cohort-level completion meaning. Service failure/block totals are not defined by this disposition model.');
  metric(p,'Action needed',model.action.length,model.action);text(p,'Overlapping condition: recorded Human Review or authority review requires operator attention.');
  p.append(el('h3','Pricing'));const pricing=el('div',undefined,'ops-overview-metrics');p.append(pricing);
  text(pricing,`Pricing targets: ${model.price.targets}`);metric(pricing,'Pricing established',model.price.established,model.price.establishedServices);metric(pricing,'Pricing unresolved / partial',model.price.unresolved+model.price.partial,model.price.unresolvedServices);metric(pricing,'No recorded pricing target (services)',model.price.noTarget.length,model.price.noTarget);
  text(p,`Unresolved targets: ${model.price.unresolved} · Partial targets: ${model.price.partial} · Other target states: ${model.price.other} · Quarantined observations: ${model.price.quarantined}`);
  text(p,'Established target confidence: '+(Object.entries(model.price.confidence).map(([k,v])=>`${readable(k)} ${v}`).join(' · ')||'Not reported'));
  text(p,'Target counts may include several markets per service; clicking shows matching services. No recorded target does not assert that pricing is inapplicable.');
  p.append(el('h3','Why unfinished'));text(p,'Overlapping diagnostics, counted once per service per reason; not failure counts.');const reasons=el('div',undefined,'ops-overview-metrics');p.append(reasons);for(const [code,ids]of model.reasons)metric(reasons,readable(code),ids.length,ids);if(!model.reasons.size)text(reasons,'No unfinished reasons recorded.');
  p.append(el('h3','Coverage by dimension'));text(p,'Established / reported applicable. Missing dimensions are unknown, not failures; explicit not-applicable states are excluded.');
  const coverage=el('div',undefined,'ops-overview-coverage');p.append(coverage);
  for(const d of [...Object.values(model.dimensions),{label:'Pricing (targets)',established:model.price.established,applicable:model.price.targets,unknown:0,notApplicable:0}]){const row=el('div'),label=`${d.label}: ${d.established} / ${d.applicable}${d.applicable?' ('+Math.round(d.established*100/d.applicable)+'%)':''}`;text(row,label+` · Unknown ${d.unknown} · Not applicable ${d.notApplicable}`);const bar=el('progress');bar.max=d.applicable||1;bar.value=d.established;bar.setAttribute('aria-label',label);row.append(bar);coverage.append(row);}
  p.append(el('h3','Research / execution efficiency'));const e=model.efficiency;const shown=x=>typeof x==='number'?x:'Not reported';text(p,`Requests: ${shown(e.used)} / ${shown(e.budget)} · Requests/service: ${e.perService===null?'Not reported':e.perService.toFixed(2)}`);text(p,Object.entries(e.usage).map(([k,v])=>`${k}: ${shown(v)}`).join(' · '));text(p,`Services with 0 fresh requests: ${e.zero} · Services with fresh requests: ${e.active} · Activity not reported: ${e.unknownActivity}. Retained evidence can legitimately avoid fresh acquisition.`);
  p.append(el('h3','Execution health'));p.append(el('p',`Execution: ${run.status}. Execution completion does not imply research completion.`,'ops-state ops-status-'+statusTone(run.status)));const job={...run,id:run.jobId??run.id,terminal:!['RUNNING','QUEUED'].includes(run.status)};text(p,`Started: ${run.startedAt??'Not reported'} · Finished: ${run.finishedAt??'Not reported'}`);text(p,liveStatusView(job).elapsed);const diagnostic=liveStatusView(job).diagnostic;if(diagnostic)text(p,diagnostic);text(p,'Recorded snapshot. Active timing and current diagnostics remain in the live card.');
 }
 async function detail(p,id,summary){
  ...
  p.append(el('h2',r.name));const overviewRows=data.overviewServices??data.services?.rows??[];let selectOverview=null;const overview=el('section',undefined,'ops-result-overview');p.append(overview);renderOverview(overview,r,overviewRows,(label,ids)=>selectOverview?.(label,ids),data.services?.total);const workspace=el('div',undefined,'ops-detail-workspace');p.append(workspace);
  ...
  const evidenceSection=section('Evidence');text(evidenceSection,'Acquired and retained references are available in each service’s details. HTTP success alone is not verification.');selectOverview=await explorer(evidenceSection,false,data.overviewServices??data.services?.rows??null);
