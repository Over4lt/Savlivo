...
export function capabilityGuidance(key,status){
 if(status?.available===true){
  if(key==='decodo')return 'Conditional · Access/geo fallback; runtime configuration validated when needed';
...
export async function mountOperations(parent,request,isCurrent=()=>true){
 ...
 async function researchBuilder(p,summary){
  ...
  text(caps,'Actually used: no run started. Run details report measured usage separately from permission. OFF prohibits use; ON never forces use. Readiness is a server prerequisite check; Preflight rechecks enabled tools before Start.');
  ...
  const submitPreflight=async(recovery=null)=>{
   ...
   try{const operation=recovery??await post('preflight',{objective:'MATURE_LIFECYCLE',scope:'SELECTED_SERVICES',services:selection.services,targeting:selection.targeting,targetingRevision:selection.revision,capabilities:requestedCapabilities});
    ...
    if(pre.liveReady===true&&pre.conflicts?.length===0){const start=button(result,'Confirm and queue run',async()=>{if(generation!==epoch)return;if(!window.confirm(`Start research for these ${selection.selectedServices} services, with a ceiling of ${pre.totalSafetyCeiling} new external requests?`))return;text(result,'Start confirmed. Revalidating before enqueue; use Recover Start if disconnected.');const job=await confirmedStart(pre.token);result.replaceChildren(el('p','Admission recorded. Follow Live job status.'));});start.setAttribute('aria-label','Confirm and queue run');}
    else text(result,'Starting is blocked. Resolve the readiness or conflict information above, then run Preflight again.');
   }catch(e){if(generation===epoch)text(result,e.message+' No research started. Use Recover Preflight after reconnecting/signing in.');}finally{enablePreflight();}
  };
