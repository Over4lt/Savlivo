...
test('live card shows recorded terminal cause and phase without loading result snapshots',async t=>{
 ...
 await button(root,'Preflight').listeners.click();window.confirm=()=>true;await button(root,'Confirm and queue run').listeners.click();await flushPulse();
 const card=descendants(root).find(n=>n.className?.startsWith('ops-live-card'));assert(text(card).includes('Cause: WORKER_SPAWN_FAILED'));assert.equal(descendants(card).find(n=>n.textContent.startsWith('Cause:')).className,'ops-state ops-status-error');assert.equal(descendants(card).find(n=>n.textContent.startsWith('Job job:')).className,'ops-state ops-status-error');assert(text(card).includes('Last recorded phase: PREPARATION_COMPLETED'));assert(!calls.some(c=>/^v2-operations\/runs\/[^?]+/.test(c.p)));assert.equal(calls.filter(c=>c.p.endsWith('/start')).length,1);
});

test('recorded result overview filters the existing service cards locally and resets',async t=>{
 const root=setup(),calls=[],rows=[{service:'ready',name:'Ready',researchComplete:true,state:'PARTIAL',pricing:[{status:'ESTABLISHED',confidence:'HIGH'}]},{service:'review',name:'Review',state:'HUMAN_REVIEW_REQUIRED',humanReview:['OWNERSHIP_REVIEW'],unresolvedReasons:['DISCOVERY_EXHAUSTED'],pricing:[{status:'UNRESOLVED'}]},{service:'pending',name:'Pending',state:'UNRESOLVED',unresolvedReasons:['BUDGET_EXHAUSTED']}],run={id:'overview',name:'Recorded cohort',status:'COMPLETE',objective:'MATURE_LIFECYCLE',requests:3};
 const dispose=await mountOperations(root,async(p,o)=>{calls.push({p,o});if(p.endsWith('/summary'))return summary;if(p==='v2-operations/runs/overview')return {run,overviewServices:rows,services:{total:3,rows},events:[]};if(p.startsWith('v2-operations/runs?'))return {rows:[run],total:1};return {rows:[],total:0};});t.after(dispose);
 await button(root,'Runs').listeners.click();await workspaceDetails(root).listeners.click();const before=calls.length;
 const names=()=>descendants(root).filter(n=>n.className==='ops-result-card').map(n=>n.children[0].children[0].textContent);
