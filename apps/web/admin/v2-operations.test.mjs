...
async function builder(t,overrides={}){const {selectTargeting}=await import('../../../services/api/src/v2-operations/targeting.mjs');const root=setup(),calls=[];const dispose=await mountOperations(root,async(p,options)=>{const body=options?.body?JSON.parse(options.body):null;calls.push({p,body});if(p.endsWith('/summary'))return {...summary,lifecycleConfigured:true,flags:{read:true,control:true,scheduling:false},capabilityAvailability:availability};if(p.endsWith('/targeting'))return {...targetModel,...selectTargeting(targetModel,body??{})};if(p.endsWith('/preflight'))return {token:'safe-token',servicesConsidered:body.services.length,totalSafetyCeiling:100,liveReady:true,conflicts:[],storage:{admissionAllowed:true,researchDisk:{admissionAllowed:true}},capabilityCheck:{capabilities:body.capabilities,availability},preflight:{retainedTargets:2},...overrides};if(p.endsWith('/start'))return {id:'job',status:'QUEUED'};return {rows:[],total:0};});t.after(dispose);return {root,calls};}
