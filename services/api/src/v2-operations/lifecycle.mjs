...
export function lifecyclePlan(settings,input){
 ...
 const derived={...sourceConfig,executionServices:selected},directory=safePath(settings.repo,'.savlivo/v2-operations-inputs');fs.mkdirSync(directory,{recursive:true,mode:0o700});const file=path.join(directory,hash(derived)+'.json');if(!fs.existsSync(file))fs.writeFileSync(file,JSON.stringify(derived,null,2),{flag:'wx',mode:0o600});else if(hash(json(file))!==hash(derived))throw Error('LIFECYCLE_INPUT_CHANGED');
 ...
 if(p.networkCalls!==0||p.onlineStarted!==false||!p.liveReady)throw Error('INVALID_LIFECYCLE_PREFLIGHT');
 return {config:{objective:'MATURE_LIFECYCLE',scope:input.scope,services:input.services??[]},manifest:{objective:'MATURE_LIFECYCLE',catalog:[],targets:[],lifecycle:{input:file,output:p.output,inputHash:hash(fs.readFileSync(file,'utf8'))},limits:{totalRequests:p.maximumNewRequests}},source:null,catalogHash:hash(p),actionable:p.servicesSelected,servicesConsidered:p.servicesSelected,servicesExpectedNetwork:null,totalSafetyCeiling:p.maximumNewRequests,baselineExcluded:p.baselineExcluded,reviewed:p.reviewed,humanReview:p.humanReview,discovery:true,decodo:true,browser:false,runner:'mature lifecycleMain',checkpoint:output,preflight:p};
}
