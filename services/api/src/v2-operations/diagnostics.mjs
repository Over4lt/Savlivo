...
export function childDiagnostic(result){
 ...
 return {stage:'native-check',reason,exitStatus:Number.isInteger(result.status)?result.status:null,signal:['SIGTERM','SIGKILL','SIGABRT'].includes(result.signal)?result.signal:null,code:operationsDiagnostic(result.error).code??childCode};
}
