...
import {jobStatus,terminationDiagnostic} from './job-status.mjs';
...
export async function operationsRequest({method,url,body,actor},ops=operations()){
 ...
 if(['GET','POST'].includes(method)&&route==='/targeting')return ops.targeting(method==='POST'?body??{}:{});
 ...
 if(method==='GET'&&parts[0]==='runs'&&parts.length===2){const r=ops.run(parts[1]),all=ops.services({runId:r.id,limit:Number.MAX_SAFE_INTEGER}),services={...all,rows:all.rows.slice(0,100)},overviewServices=all.rows.map(({service,name,state,authority,identity,subscription,markets,login,management,cancellation,pricing,humanReview,unresolvedReasons,reason,requests,researchComplete,evidenceCount,changed,eligible,eligibilityReason})=>({service,name,state,authority,identity,subscription,markets,login,management,cancellation,pricing,humanReview,unresolvedReasons,reason,requests,researchComplete,evidenceCount,changed,eligible,eligibilityReason}));const recordedJob=ops.db().jobs.find(j=>j.id===r.jobId);return {run:{...publicRun(r),...(recordedJob?{startedAt:recordedJob.startedAt??null,finishedAt:recordedJob.finishedAt??null,diagnostic:terminationDiagnostic(recordedJob,null,null)}:{})},services,overviewServices,efficiency:{requestsPerFreshService:r.freshResolutions>0&&r.requests!==null?r.requests/r.freshResolutions:null,freshPer100Requests:r.requests>0&&r.freshResolutions!==null?r.freshResolutions*100/r.requests:null},events:[...ops.db().events.filter(e=>e.jobId===r.jobId).slice(-100),...(r.lifecycle?lifecycleEvents(r._directory):[])]};}
