...
export function deploymentPreflight(env=process.env){
 ...
 if(config.lifecycleInput&&process.cwd()===config.repo)try{plan=lifecyclePlan(config,{objective:'MATURE_LIFECYCLE',scope:'FULL_CATALOG',services:[]});check('matureOfflinePreflight',plan.liveReady);}catch{check('matureOfflinePreflight',false);}
