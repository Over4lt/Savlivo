...
for(const scenario of ['embedded','missing-market','missing-authority','valid','reported-exact','reported-prefixed'])test('real retained interpreter and targeted verifier: '+scenario,()=>{
 ...
 if(scenario==='valid'||real){if(!real)for(const amount of ['11.99','21.99'])assert.deepEqual([...new Set(discovery.filter(c=>c.sourceType==='HTML'&&c.amountNormalized===amount).map(c=>c.currencyRaw))].sort(),['EUR','€']);assert.equal(read('complete').monthlyStrong,3);assert.deepEqual(report.verified.map(v=>v.amount).sort(),[...values,'39']);assert(plans.filter(p=>p.strongRecurringMonthly).every(p=>p.monthlyPlanId));}else assert.equal(report.verified.length,0);
