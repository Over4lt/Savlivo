...
export function semantics(text,local,original=false){const periods=[];if(/\b(?:month(?:ly)?|mo|monat|mensuel|mensual|mensal|maand|mese|miesiąc|måned|månad)|μήν|tháng|เดือน|月|شهري/iu.test(local))periods.push('MONTH');if(/\b(?:year(?:ly)?|annual(?:ly)?|annuel|anual|jahr|año|jaar|anno|år)\b|έτος|năm|年/iu.test(local))periods.push('YEAR');
 ...
 const cadence=billingCadence(local);return {billingInterval:cadence.interval,cadenceFamily:cadence.interval?.cadenceFamily??null,billingPeriod:cadence.interval?.normalized??(periods.length===1?periods[0]:null),billingPeriodAmbiguous:periods.length>1||cadence.ambiguous,qualifier:qualifiers,promotionOrTrial:original?'REFERENCE_PRICE':promo,qualifierAmbiguous:periods.length>1||(/\b(?:free|trial|offer|promo)\b/i.test(text)&&!promo&&!original),commercialContext:local};}
...
function applyProse(c,r,{bodyHash,path}){
 ...
 c.promotionOrTrial=c.qualifier.includes('REFERENCE_PRICE')?'REFERENCE_PRICE':r.phase==='INTRO'?'PROMOTION_OR_TRIAL':r.phase==='INCLUDED'?'INCLUDED_BENEFIT':null;
...
export function extract(body){if(Buffer.byteLength(body)>4000000)throw Error('SOURCE_BODY_BOUND');const result=[],tree=htmlTree(body);let jsonCount=0,proseFields=0,proseCharacters=0,proseBodyHash;const diagnostics={malformedHtml:tree.malformed,unsupportedScriptPayloads:0,jsonPayloads:0};
 ...
 for(const n of tree.nodes){if(n.tag==='#text'||skip.has(n.tag)||['root','head','html','body'].includes(n.tag))continue;const ms=cached.get(n);for(const m of ms){if(n.children.some(ch=>ch.tag!=='#text'&&cached.get(ch)?.some(x=>x.raw===m.raw)))continue;
   const prose=!isHidden(n)&&namedPriceProse(n.text,{monetary}).find(r=>r.amountSpan[0]===m.start&&r.amount===m.amount);
   ...
   if(prose){owned={name:prose.plan,node:n,method:'NAMED_PRICE_PROSE',strong:true};local=prose.raw;}
   ...
   }if(tableCell){const {node,...proof}=tableCell;c.billingTableEvidence={...proof,bodyHash:hash(body)};}if(sectionFee){const {node,...proof}=sectionFee;c.labelledSectionEvidence={...proof,bodyHash:hash(body)};}if(declaration)c.subscriptionDeclaration={...declaration,path:pathOf(n),bodyHash:hash(body),span:[n.start,n.end]};c.rawEvidenceSnippet=body.slice(n.start,n.end).slice(0,6000);c.structuredPath=pathOf(n);c.discoveryCoordinates={start:n.start,end:n.end,coordinate:'UTF16_RETAINED_BODY_NEW_ANALYSIS'};c.siblingLabels=(owned.node.headings??[]).map(h=>h.text);c.sourceType='HTML';if(n.tag==='img')c.semanticAttribute={name:'alt',value:n.attrs.alt,path:pathOf(n),span:[n.start,n.openEnd],basis:'ACTIVE_IMAGE_ALTERNATIVE_TEXT'};if(!prose)preserveQualifiers(c,{localText:declaration?.raw??tableCell?.raw??sectionFee?.raw??n.text,ownerText:declaration?.raw??tableCell?.raw??sectionFee?.context??owned.node.text,localPath:pathOf(n),ownerPath:pathOf(owned.node),ownerStrong:owned.strong,localMoneyCount:ms.length,ownerMoneyCount:cached.get(owned.node)?.length??0});if(prose){preserveQualifiers(c,{localText:prose.raw,ownerText:prose.raw,localPath:pathOf(n),ownerPath:pathOf(n),ownerStrong:true});applyProse(c,prose,{bodyHash:proseBodyHash??=hash(body),path:pathOf(n)});}result.push(c);
  }}
