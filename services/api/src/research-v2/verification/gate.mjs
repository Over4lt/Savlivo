...
export const gateVersion='V2_FIELD_VERIFICATION_V1';
...
export function deriveEvidence(body,context){
 ...
 const rows=raw.candidates.map((r,i)=>{
  ...
  boundedOwnership(c,source);const old=structuredClone(c);scopeOfferRole(c,source);return {c,old};
 });
 ...
 return {rows:rows.map(r=>r.c).filter((c,i,all)=>!c.namedOfferDetails||all.findIndex(x=>x.namedOfferDetails&&x.structuredPath===c.structuredPath&&x.amountNormalized===c.amountNormalized&&x.currencyRaw===c.currencyRaw)===i),bodyHash:ctx.bodyHash};
}
...
function fieldsFor(claim,d,acquisition){
 ...
 const conditions={
  ...
  plan:samePlan&&!!owner?.path&&!!owner.raw&&!d?.ownershipAmbiguous&&!rawReasons.includes('CTA_OR_TRIAL_HEADING_NOT_PLAN')&&!(commercial?.reasons??[]).includes('CTA_OR_TRIAL_HEADING_NOT_PLAN'),
  ...
  ordinaryPriceRole:commercial?.ordinaryMonthly===true&&!commercial.monthlyBlockers.includes('NOT_PRINCIPAL_MONTHLY_CHARGE')&&(!d.offerRole||['REGULAR_BASE','POST_INTRO_REGULAR','UNKNOWN'].includes(d.offerRole.role)),
  ownership:samePlan&&!!owner?.path&&!d?.ownershipAmbiguous&&!d?.crossCardRisk&&a?.productOwnershipEstablished===true,
...
export function verifyIdentity(plan,candidateDecisions,{canonicalConflict=false}={}){
 ...
 const blockers=unique([...(good.length?[]:supports.flatMap(s=>s.blockers)),...(!supports.length?['PROVENANCE_INTEGRITY_FAILURE']:[]),...(conflict?['UNRESOLVED_CRITICAL_CONFLICT']:[])]);
