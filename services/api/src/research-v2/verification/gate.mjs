...
export function verifyCandidate(claim,derived,acquisition){
 ...
 const witnesses=claim.discoveryOccurrences?.length?claim.discoveryOccurrences:[{structuredPath:claim.structuredPath}];
 const occurrences=witnesses.map(witness=>{const path=witness.structuredPath;const matches=(derived?.rows??[]).filter(d=>d.sourceType===claim.sourceType&&d.structuredPath===path&&d.amountNormalized===claim.amountNormalized&&d.currencyRaw===claim.currencyRaw&&sameProseEvidence(d,witness.prosePriceRelationship?{...claim,prosePriceRelationship:witness.prosePriceRelationship}:claim));const d=matches.length===1?matches[0]:null;return {path,offerPresentation:d?.offerPresentation??null,billingInterval:d?.commercial.billingInterval??null,cadenceFamily:d?.commercial.cadenceFamily??null,derivedProduct:d?.product??null,derivedCommercialType:d?.commercial.type??null,fields:fieldsFor(claim,d,acquisition)};});
