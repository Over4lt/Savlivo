...
export function compoundOfferMarket(candidate,resource,context){
 ...
 const locale=pp?.locale,country=typeof locale==='string'?region(locale):null;
 if(pp?.preview!==false||country!==context.market||page?.sys?.locale!==locale||page?.sys?.contentType?.sys?.id!=='page')return null;
 ...
 const evidence=[{...base,type:'COMPOUND_REGIONAL_COMMERCIAL_OFFER',negative:false,reviewStatus:'ESTABLISHED',scopeSubject:candidate.product,qualificationType:'REGIONAL_PURCHASE_CHANNEL',binding:{pagePath:pathOf(script)+'/props/pageProps/page',localePaths:['/props/pageProps/locale','/props/pageProps/page/sys/locale'],slug,legalTextPath:candidate.structuredPath,purchaseSpan:[text.indexOf(purchase),text.indexOf(purchase)+purchase.length],coordinate:'NORMALIZED_SOURCE_TEXT_UTF16',offer:r.relationship}}];
