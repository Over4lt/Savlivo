...
import {safe,relative,read,sha,digest,stableBytes,stableHash,stableJsonLines,releaseValidationTemporaries,jsonDigest,statFile,files,sealed,verifySeal,immutable,canonical,CLASSES} from './core.mjs';
...
export function closure(root,roots,{maxFiles=100000,maxBytes=4*1024**3,genesisArchive=false}={}){
 ...
 const journalCache=new Map();let journalCacheBytes=0;
 const entries=new Map(),errors=[],pending=roots.map(r=>({...r,reason:r.reason??'EXPLICIT_ROOT'}));let total=0,currentReference=null,currentField=null,cursor=0,lastCollectionAt=0;
   ...
   if(typeof v==='string'){
    ...
    if(pathKeys.has(key)||looksPath){currentField=key;
     ...
     if(key==='path'&&/\/journal\/record-\d{8}\.json$/.test(name)&&typeof value.pointer==='string'&&(value.hash||value.recordHash)&&!value.sha256){
      ...
      if(!cached){const b=stableBytes(f);cached={identity,record:JSON.parse(b),byteHash:sha(b),bytes:b.length};while(journalCache.size&&(journalCacheBytes+b.length>4*1024*1024||journalCache.size>=16)){const key=journalCache.keys().next().value;journalCacheBytes-=journalCache.get(key).bytes;journalCache.delete(key);}if(b.length<=4*1024*1024){journalCache.set(name,cached);journalCacheBytes+=b.length;}}
      const record=cached.record,{hash,...payload}=record;if(cached.sealValid===undefined)cached.sealValid=hash===jsonDigest(payload);
 ...
 try{while(cursor<pending.length){if(entries.size-lastCollectionAt>=128){releaseValidationTemporaries();lastCollectionAt=entries.size;}let ref=pending[cursor];pending[cursor++]=null;currentReference=ref.path;if(!CLASSES.includes(ref.classification))throw Error('STORAGE_UNKNOWN_CLASS');const file=safe(root,ref.path);
   ...
   if(ref.path.endsWith('.lock')&&!terminalLocks.has(ref.path))throw Error('STORAGE_LOCK_REFERENCE_UNSAFE');const before=statFile(file),streamed=['BYTES','CODE','JSONL'].includes(ref.format),bytes=streamed?null:stableBytes(file),hash=streamed?stableHash(file):sha(bytes);if(before.identity!==statFile(file).identity)throw Error('STORAGE_PUBLICATION_RACE');if(ref.sha256&&hash!==ref.sha256)throw Error('STORAGE_REFERENCE_HASH:'+ref.path);
   ...
   total+=before.bytes;if(entries.size>=maxFiles||total>maxBytes)throw Error('STORAGE_CLOSURE_LIMIT');
   entries.set(ref.path,{path:ref.path,sha256:hash,...before,classification:ref.classification,format:ref.format,reasons:[ref.reason]});
   ...
   if(ref.format==='SNAPSHOT'){const s=JSON.parse(bytes),{snapshotHash,...p}=s;const legacy=jsonDigest(p);if(!Object.hasOwn(s,'snapshotHash'))throw Error('STORAGE_LEGACY_SEAL_MISSING');if(s.version!==1||snapshotHash!==legacy)throw Error('STORAGE_LEGACY_SNAPSHOT_HASH');
   ...
   const values=ref.format==='JSON'?[JSON.parse(bytes)]:stableJsonLines(file,before.identity);
