...
export function closure(root,roots,{maxFiles=100000,maxBytes=4*1024**3}={}){
 const entries=new Map(),errors=[],pending=roots.map(r=>({...r,reason:r.reason??'EXPLICIT_ROOT'}));let total=0,currentReference=null,currentField=null;
 ...
 try{while(pending.length){const ref=pending.shift();currentReference=ref.path;if(!CLASSES.includes(ref.classification))throw Error('STORAGE_UNKNOWN_CLASS');const file=safe(root,ref.path);
