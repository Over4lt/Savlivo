...
export function retainedPriceReview(directory,target){
 ...
 const price=(JSON.parse(fs.readFileSync(artifact)).observations??[]).find(o=>o.service===target.service&&['HIGH','MEDIUM'].includes(o.confidence)&&o.source?.kind==='ORIGINAL_PROVIDER'&&(o.market===target.market||target.researchObjective==='SERVICE_COVERAGE'));
 return price?{sourceBound:true,confidence:price.confidence,market:price.market,objective:target.researchObjective,sourceHash:price.source.hash,artifact}:null;
}
...
export async function runOpenWebResearch({directory,targets,search,read,classify,acquire,consumeProvider,bounds,catalog,maxActions=Infinity,onSourceObservation}){
 ...
 if(authority.eligible===true&&consumeProvider&&t.researchObjective!=='CATALOG_ONLY'){t.decisions.at(-1).directInterpretationReserved=true;session.save();direct=await consumeProvider({target:t,candidate:{url:a.url,authority},page,directory});session.event('PROVIDER_INTERPRETATION',{target:t.id,url:a.url,outcome:direct});t.verified.push(...(direct.verified??[]));mergePriceEvidenceNeeds(t,direct.priceEvidenceNeeds);t.providerInterpretations??=[];t.providerInterpretations.push({url:a.url,classification:direct.classification,blockers:direct.blockers??[],needsGeo:direct.needsGeo??false,runDirectory:direct.runDirectory??null});if([1,2].includes(t.smartResearch?.version)){t.researchDiagnosis=diagnoseResearch({authority:true,target:true,priceCount:direct.monetary??0,blockers:direct.blockers??[],failure:page.failure?.code??null,configurator:direct.configuratorRequired===true,rendering:direct.renderingRequired===true});t.demonstratedConfigurator=direct.configuratorRequired===true;t.demonstratedRendering=direct.renderingRequired===true;
