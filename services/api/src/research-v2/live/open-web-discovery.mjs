...
export class OpenWebResearch {
 ...
 else if(t.researchPlan?.route==='DECODO'&&executableAction(t,t.researchPlan,{bounds:b,usage:s.usage}).executable)action={type:'ACQUIRE',target:t.id,url:t.researchPlan.url,route:'DECODO',reason:t.researchPlan.reason};
...
export async function runOpenWebResearch({directory,targets,search,read,classify,acquire,consumeProvider,bounds,catalog,maxActions=Infinity,onSourceObservation}){
 ...
 if(a.type==='ACQUIRE'){
  ...
  if(!gate.executable)throw Error(gate.reason);
  if(typeof classify!=='function'||typeof acquire!=='function')throw Error('DECODO_ADAPTER_REQUIRED');
  const authority=await classify({target:t,page:{url:a.url,requestedUrl:a.url},directory});
  session.recordAuthority(t.id,a.url,authority);
  if(authority.eligible!==true){t.done=authority.reason??'AUTHORITY_UNRESOLVED';session.state.pending=null;session.save();if(++executed>=maxActions)return session.state;continue;}
  session.state.usage.acquisitions++;Object.assign(t.decisions.at(-1),{acquisitionReserved:true,route:a.route,routingReason:a.reason});session.save();
  const outcome=await acquire({target:t,candidate:{url:a.url,authority},directory,route:a.route,reason:a.reason});
  session.event('AUTHORITATIVE_RESULT',{target:t.id,url:a.url,route:a.route,reason:a.reason,outcome});
  t.verified.push(...(outcome.verified??[]));
  if(outcome.quarantinedVerified)t.quarantinedVerified=[...new Map([...(t.quarantinedVerified??[]),...outcome.quarantinedVerified].map(q=>[JSON.stringify(q),q])).values()];
 ...
 let direct=null;
 if(authority.eligible===true&&page.outcome==='OK'&&page.bodyFile){observeAdmissionSource(t,page,fs.readFileSync(directory+'/'+page.bodyFile,'utf8'),{path:directory+'/'+page.bodyFile,hash:page.bodyHash});session.save();}
 ...
 if(authority.eligible===true&&consumeProvider&&t.researchObjective!=='CATALOG_ONLY'){t.decisions.at(-1).directInterpretationReserved=true;session.save();direct=await consumeProvider({target:t,candidate:{url:a.url,authority},page,directory});session.event('PROVIDER_INTERPRETATION',{target:t.id,url:a.url,outcome:direct});t.verified.push(...(direct.verified??[]));if(direct.quarantinedVerified)t.quarantinedVerified=[...new Map([...(t.quarantinedVerified??[]),...direct.quarantinedVerified].map(q=>[JSON.stringify(q),q])).values()];mergePriceEvidenceNeeds(t,direct.priceEvidenceNeeds);acceptMarketResearch(t,direct,l=>session.add(t,l));t.providerInterpretations??=[];t.providerInterpretations.push({url:a.url,classification:direct.classification,blockers:direct.blockers??[],needsGeo:direct.needsGeo??false,runDirectory:direct.runDirectory??null});if([1,2].includes(t.smartResearch?.version)){t.researchDiagnosis=diagnoseResearch({authority:true,target:true,priceCount:direct.monetary??0,blockers:direct.blockers??[],failure:page.failure?.code??null,configurator:direct.configuratorRequired===true,rendering:direct.renderingRequired===true});t.demonstratedConfigurator=direct.configuratorRequired===true;t.demonstratedRendering=direct.renderingRequired===true;
