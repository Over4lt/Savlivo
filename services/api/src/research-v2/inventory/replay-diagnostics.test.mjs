...
test('oversized persisted projection SIZE_BOUND retains last success and bounded diagnostics',async t=>{
 const f=fixture(t,2),before=structuredClone(f.target);let calls=0;
 const e=await fails(f,async args=>{const i=calls++;return {verified:[{sourceHashes:[args.page.bodyHash]}],priceEvidenceNeeds:i===0?needs(args.target,args.page):{...needs(args.target,args.page),untrustedOversizedField:'x'.repeat(2200000)},runDirectory:null};});
 assert.equal(e.message,'PRICE_NEEDS_SIZE_BOUND');assert.equal(calls,2);const p=f.progress();assert.equal(p.stage,'PRICE_EVIDENCE_NEEDS_MERGE');assert.equal(p.current.ordinal,2);assert.equal(p.current.recordPersisted,true);assert.equal(p.lastIntegrated.ordinal,1);assert.equal(p.lastIntegrated.sourceHash,f.pages[0].bodyHash);assert.equal(p.lastIntegrated.integrationComplete,true);assert.equal(p.metrics.prior.sources,1);assert.equal(p.metrics.incoming.sources,1);assert.equal(p.metrics.aggregateByteLimit,2097152);assert(p.metrics.prior.serializedBytes<10000);assert(p.metrics.incoming.serializedBytes>2097152);assert(Buffer.byteLength(JSON.stringify(p))<5000);assert(!JSON.stringify(p).includes('xxxxx'));assert.deepEqual(f.target,before);assert(!fs.existsSync(f.directory+'/retained-replay.json'));
...
test('large planning locators do not abort durable successful replay results',async t=>{
 const f=fixture(t,7);let calls=0;const result=await replayTarget({...f,interpret:async args=>{calls++;return {verified:[{sourceHashes:[args.page.bodyHash]}],priceEvidenceNeeds:needs(args.target,args.page),runDirectory:null};}});
 assert.equal(calls,7);assert.equal(result.target.verified.length,7);assert(result.target.priceEvidenceNeeds.coverage.complete);assert(Buffer.byteLength(JSON.stringify(result.target.priceEvidenceNeeds))<50000);assert.equal(fs.readdirSync(f.directory).filter(n=>/^price-.*\.json$/.test(n)).length,7);assert(fs.existsSync(f.directory+'/retained-replay.json'));assert.equal(f.progress().status,'COMPLETE');assert.equal(result.networkCalls,0);assert.equal(result.target.requestCeiling,36);assert.deepEqual(result.target.priorUsage,f.target.priorUsage);
});
