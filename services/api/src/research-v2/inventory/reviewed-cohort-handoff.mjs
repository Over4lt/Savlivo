...
export function inspectLifecycleInput(file){
 const config=read(file),manifest=read(config.cohortManifest),universe=read(config.universe),document=read(config.reviewedBindings),ids=manifest.serviceIds,candidates=[...universe.new_include,...universe.research];
 if(config.version!==1||!config.frozenHashes||Object.entries(config.frozenHashes).some(([f,h])=>shaFile(f)!==h)||!config.frozenHashes[config.cohortManifest]||!config.frozenHashes[config.universe])throw Error('LIFECYCLE_FROZEN_INPUT');
 ...
 const derived=reviewedTargets(candidates,document),work=config.providerWorklist?read(config.providerWorklist):[],rows=ids.map(id=>({service:id,providerReview:work.find(w=>w.service===id)?.providerReview??{}}));
 ...
 let historicalRequests=0;const files=[file,config.cohortManifest,config.universe,config.reviewedBindings,...[config.providerWorklist,config.researchScopes,config.quarantineFile].filter(Boolean)];
 if(config.legacyDirectory){const ledger=config.legacyDirectory+'/network.jsonl',checkpoint=config.legacyDirectory+'/checkpoint.json',events=fs.readFileSync(ledger,'utf8').trim().split('\n').filter(Boolean).map(JSON.parse),state=read(checkpoint);if(events.some(e=>!ids.includes(e.service))||state.requests!==events.length)throw Error('LIFECYCLE_LEGACY_LEDGER');historicalRequests=events.length;files.push(ledger,checkpoint);}
 return {config,baselineIds:baseline,cohort:{manifest,candidates},document,targets:derived.launchTargets.filter(t=>!config.executionServices||config.executionServices.includes(t.service)),rows:rows.filter(r=>!config.executionServices||config.executionServices.includes(r.service)),inputHashes:Object.fromEntries(files.map(f=>[f,shaFile(f)])),summary:{cohort:ids.length,baselineExcluded:baseline.length,historicalRequests}};
}
